...
// always @(posedge CLK)
// begin
//     $display("T=%0t READY=%b START=%b BUSY=%b DONE=%b VALID=%b RESULT=%h STATUS=%h",
//              $time,
//              READY,
//              DUT.REGISTERS_UNIT.CTRL[0],
//              DUT.BUSY_REG,
//              DUT.DONE_REG,
//              DUT.VALID_REG,
//              DUT.RESULT_REG,
...
begin
    ...
    apb_write(8'h04,8'd20);      // Operand A
    // apb_read(8'h04,result);
    // $display("data at 04 = %d",result);
    apb_write(8'h08,8'd10);      // Operand B
    apb_write(8'h0C,8'h00);      // ADD opcode
    apb_write(8'h00,8'h01);      // START

    // Wait until ALU completes
    // Wait until ALU completes by polling STATUS
    status = 8'h00;
    ...
    $display("----------------------------------------");
    $display("RESULT = %d (Hex : %h)",result,result);
    $display("----------------------------------------");
    ...
    apb_write(8'h04,8'd20);      // Operand A
    apb_write(8'h08,8'd10);      // Operand B
    apb_write(8'h0C,8'h01);      // sub opcode
    apb_write(8'h00,8'h01);      // START
    ...
    // $display("data at 04 = %d",result);
    apb_write(8'h08,8'd10);      // Operand B
    apb_write(8'h0C,8'h02);      // mul opcode
    apb_write(8'h00,8'h01);      // START
    ...
    // Correct Programming Order
    apb_write(8'h04,8'd20);      // Operand A
    apb_write(8'h08,8'd5);      // Operand B
    apb_write(8'h0C,8'h03);      // div opcode
    apb_write(8'h00,8'h01);      // START
